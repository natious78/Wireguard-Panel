...
import { bandwidthBpsFromInput, peerCreateSchema, quotaBytesFromInput } from "@/lib/validation";
import { createPeer } from "@/server/peer-service";

export async function POST(request: NextRequest) {
  const auth = await requireUser("peer:create");
  ...
  try {
    ...
    const bandwidth = bandwidthBpsFromInput(input);
    const peer = await createPeer({ ...input, ...bandwidth, quotaBytes, quotaPeriod: quotaBytes ? input.quotaPeriod : null, userId: auth.user.id });
