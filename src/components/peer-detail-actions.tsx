...
export function PeerDetailActions({peer,routers,interfaces,pools}:{peer:Peer;routers:RouterOption[];interfaces:InterfaceOption[];pools:PoolOption[]}) {
  ...
  return <>
    <div className="peer-action-layout"><div className="actions peer-primary-actions">
      ...
      {peer.managed&&<button className="button" onClick={()=>setPending("regenerate")}><KeyRound/>Rotate WireGuard keys</button>}
...
function confirmationCopy(value:"disable"|"delete"|"reset_usage"|"temporary_reenable"|"regenerate",peer:Peer){
  ...
  if(value==="regenerate")return{title:"Rotate WireGuard keys?",description:"A new client key pair will replace the existing identity on MikroTik.",details:<p>The previous WireGuard configuration will stop working immediately. Download or scan the newly generated configuration after rotation.</p>,confirmLabel:"Rotate keys",tone:"danger" as const};
